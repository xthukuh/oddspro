...
- `src/hotpicks.js` — over-2.5 hot picks + best tips (`updateHotPicks()`): settles `fixture_predictions` outcomes (hot picks via SQL, tips via pure `tipHit`) from canonical final scores, then re-evaluates every upcoming correlated snapshot-backed fixture through the deduction gates; rule-hot candidates optionally pass an OpenRouter confirm/veto (`src/ai.js`, fail-open — errors keep the rule verdict; verdicts are reused while the score is unchanged so reruns don't re-bill). Same freeze idiom as prematch: `kickoff > NOW()` selection, chunked onConflict-merge upsert (merge list excludes `result_goals`/`outcome` — owned by the settle pass). `hotpicksSummary()` backs `GET /api/hotpicks`.
...
- `src/db/tip-rules.js` — pure "Tip" column deduction (zero imports): `teamOutcomeAggregates`/`h2hOutcomeAggregates` (W/D/L + per-line over rates, kickoff cutoff), `bestTip` — safest bettable outcome across 1X2/double-chance/every O/U line, confidence = renormalized blend of devigged market probability (0.6) + rolling-stats support (0.3) + API-Football percentages (0.1), with a price floor (`TIP_MIN_PRICE`, default 1.2) that excludes near-certain junk odds — what survives at high confidence is the "hidden gem". `tipHit` settles any canonical market from a final score. Tips live on `fixture_predictions` (`tip_market/tip_price/tip_confidence/tip_outcome`), freeze at kickoff, sort/filter via the `tip` base column (confidence).
