...
- [~] Step 1: `tipEligibility()` pure gate in `src/db/tip-rules.js` — scaffold + integration + contract tests DONE; the ~8-line rule body is a **pending user contribution** (placeholder passes everything through, so live behavior is unchanged until it lands; the 5 contract tests in `tests/tip-rules.test.js` are `todo`-marked — un-mark them with the body)
