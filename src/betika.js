...
function _date(value, set_hours_=undefined) {
    const date = value
...
async function _batch(list, each, parallel=5) {
        ...
        const _next = () => {
            ...
            .catch(e => {
                if (!abort) {
                    abort = 1;
                    error = e;
                    warn(`[${index}/${len}] Exception: ${e.code ? `[${e.code}] ` : ''}${e?.message || e}`);
                }
            })
            .finally(() => {
                pending--;
