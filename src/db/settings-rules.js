...
export const SETTINGS_CATALOG = [
    ...
    { key: 'AUTO_FULL_MAX_ATTEMPTS', type: 'int', group: 'refresh', public: false, live: true, min: 1, unit: 'attempts', label: 'Daily sweep retry cap', hint: 'A full sweep that errors or partially fails may retry this many times the same EAT day before waiting for tomorrow; a successful (or partially successful) run always stops retries immediately.' },
    ...
    // ---- Warm keeper (src/warm.js) - late-read every 5s tick, all live.
    { key: 'WARM_ENABLED', type: 'boolean', group: 'refresh', public: false, live: true, label: 'Warm keeper', hint: 'Precompute the heavy read payloads (records/columns/hotpicks/performance/magic-sort) after every data refresh so no visitor pays a cold compute.' },
    { key: 'WARM_DATES_BACK', type: 'int', group: 'refresh', public: false, live: true, min: 0, unit: 'days', label: 'Warm: days behind', hint: 'How many days before today to keep precomputed in the records cache.' },
    { key: 'WARM_DATES_AHEAD', type: 'int', group: 'refresh', public: false, live: true, min: 0, unit: 'days', label: 'Warm: days ahead', hint: 'How many days after today to keep precomputed in the records cache.' },
    { key: 'WARM_MAX_AGE_MINUTES', type: 'number', group: 'refresh', public: false, live: true, min: 1, unit: 'min', label: 'Warm: re-warm cadence', hint: 'Force-recompute warmed payloads older than this even without a refresh, so cache entries can never expire cold. Keep under the 10-minute response-cache TTL.' },
    ...
    { key: 'WATCHDOG_FULL_SWEEP_GRACE_MINUTES', type: 'number', group: 'refresh', public: false, live: true, min: 1, unit: 'min', label: 'Watchdog: full-sweep grace', hint: 'While a full sweep is running (and is younger than this), the watchdog reports busy instead of stale: no restart, no SMS. Past it the sweep is presumed dead and the normal stale rules apply.' },
    ...
    { key: 'WATCHDOG_RESTART_COOLDOWN_MINUTES', type: 'number', group: 'refresh', public: false, live: true, min: 0, unit: 'min', label: 'Watchdog: restart cooldown', hint: 'At most one recovery restart per this many minutes; restarts stop entirely after 5 consecutive stale runs (alerts keep firing).' },
    ...
    { key: 'ENRICH_CONCURRENCY', type: 'int', group: 'pipeline', public: false, live: true, min: 1, max: 8, unit: 'fixtures', label: 'Warehouse enrichment concurrency', hint: 'How many fixtures the deep-stats / history / predictions / standings backfills process at once. 1 is the old serial behaviour. Measured 2026-08-29: 4 gives ~4.4x with no deadlocks; 8+ does deadlock occasionally, and withRetry recovers it.' },
    ...
    { key: 'AI_BLIND_MODEL_FALLBACKS', type: 'string', group: 'ai', public: false, live: true, label: 'Blind model fallbacks', hint: 'Ordered CSV tried when the blind model answers 404; each entry must still be a different vendor from the anchored model or it is dropped. Blank = no fallback.' },
    { key: 'AI_MAX_TOKENS', type: 'int', group: 'ai', public: false, live: true, min: 256, unit: 'tokens', label: 'AI reply ceiling', hint: 'Completion budget per AI call. Too low and a reasoning model spends it all thinking and returns no content.' },
    { key: 'AI_MAX_TOKENS_RETRY', type: 'int', group: 'ai', public: false, live: true, min: 256, unit: 'tokens', label: 'AI re-ask ceiling', hint: 'Raised budget for the single automatic re-ask after a truncated or content-less reply.' },
    { key: 'AI_REASONING_EFFORT', type: 'string', group: 'ai', public: false, live: true, pattern: '^(off|low|medium|high)$', patternHint: 'off, low, medium or high', label: 'AI reasoning effort', hint: 'Caps the reasoning stream so the reply reaches actual content; off omits the parameter for models that reject it.' },
    { key: 'AI_JSON_MODE', type: 'boolean', group: 'ai', public: false, live: true, label: 'Strict JSON replies', hint: 'Ask the provider for a syntactically valid JSON object; turn off only if a model rejects response_format.' },
    ...
    { key: 'GUEST_PREMIUM_EXCEPT', type: 'string', group: 'auth-policy', public: true, live: true, label: 'Guest premium exceptions', hint: 'Comma-separated feature keys kept signed-in-only even while guest premium is on. Keys: tip_reasoning, future_dates, sure_bets, safe_picks, methodology, daily_multibet. Empty = guest premium opens all of them.' },
    ...
    // ---- Bot UA filter.
    { key: 'API_DETAILS', type: 'boolean', group: 'bot', public: false, live: true, label: 'Ship tip internals over the API', hint: 'Global bandwidth kill-switch, OVERRIDES the premium registry: off = strip the heavy per-row reasoning JSON (tip breakdown, AI reviews, hot signals) from every web payload, so the tip runners-up and Why-this-tip sections render empty even for tiers entitled to them. Leave ON unless you are deliberately trading those surfaces for payload size. CLI/CSV and machine-bearer reads are never slimmed.' },
