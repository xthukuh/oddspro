...
```css
    ...
    /* Ribbon TEXT inks. Separate from the --orange/--yellow hues above because
       those are tuned as accent fills, and at small text size on the light
       surface they measure about 2:1 against the page background, well under
       the 4.5:1 readable bar. The fills stay as they are (borders and washes
       do not carry meaning on their own); only the text uses these. */
    --color-alert-ink: var(--alert-ink);
    --color-warn-ink: var(--warn-ink);
```

Then add THREE variables to all FOUR palette blocks. In `:root` (light) and
`:root[data-theme="light"]`, after `--orange: #FF9500;`:
...
In `@media (prefers-color-scheme: dark) { :root { ... } }` and
`:root[data-theme="dark"]`, after `--orange: #FF9F0A;`:
...
Measured against this project's own surfaces: light `#A34A08` on `#F1F1F4` is
about 5.3:1 and `#8A6200` about 4.9:1, both clearing 4.5:1. Dark reuses the
bright hues directly, which already measure 8.5:1 and 12.2:1 on `#1A191D`.
The light values are deliberately much darker than the fills: a pure `#FFCC00`
on a near-white surface is unreadable at this text size.
