...
**Interfaces:**
- Consumes: `eatDateKey` from `src/db/auto-rules.js` if that export exists (see Step 1), otherwise a local equivalent.
- Produces:
...
- [ ] **Step 1: Confirm whether `eatDateKey` is exported from auto-rules**

Run:
```bash
grep -n "export function eatDateKey\|export const eatDateKey" src/db/auto-rules.js
```

If it prints a match, Task 1 imports it. If it prints nothing, use the local `eatDay` defined in Step 3 and do not add the import line. Both branches are fully written below; pick one and delete the other.
...
test('healthy thin slates produce no notice', () => {
    const thin = ['2026-07-20', '2026-07-21', '2026-07-27', '2026-07-28', '2026-07-30'];
    const runs = [];
    for (const d of thin) {
        for (const hh of ['06', '07', '08']) runs.push(run(`${d}T${hh}:00:00+03:00`, 'ok', [d]));
    }
...
import { detectNotices, coveragePayload } from './db/notice-rules.js';
...
const _day = v => (v instanceof Date ? v.toISOString().slice(0, 10) : String(v ?? '').slice(0, 10));
const _json = v => {
...
In `src/db/records.js`, add near the other imports:
...
```js
    return {
        ...
        coverage: date ? coverageFor(new Date(date).toISOString().slice(0, 10)) : { status: 'ok', confirmed: true, notices: [] },
