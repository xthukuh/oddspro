...
**Goal:** Ship the approved Sure-bets feature (spec `docs/dev/specs/2026-07-17-sure-bets-filter-design.md`): a signed-in-only "Sure bets" toggle that filters the table to the day's top-10 safest tips ranked by calibrated win probability, with an N-of-10 pill, an explicit zero-day warning, and a one-tap "Top-3 slip" into the betslip playground.
...
**Files:**
- Modify: `docs/guides/safety-net-protocol.md` (append a "Sure bets" section)
...
```powershell
git add docs/guides/safety-net-protocol.md CLAUDE.md
