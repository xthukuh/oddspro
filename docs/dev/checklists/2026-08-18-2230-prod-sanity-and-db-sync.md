...
| 28 | Audit link.js and apisports.js | NEXT SESSION (owner-directed 2026-08-19) | both audit agents died on API errors this session, so neither surface has ever been reviewed. link.js: wrong-link-at-high-confidence and permanent alias poisoning, plus the ~33k unlinkable Betika virtual "Zoom" matches retried forever. apisports.js: a fetch-once flag set when nothing was stored would permanently skip real data. Brief kept in the resume point |
