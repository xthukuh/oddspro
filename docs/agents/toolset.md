...
- 2026-08-24 - **Verifying a read-layer hotfix behaviorally when SSH credential access is
  blocked (verified):** the harness classifier can refuse a remote `kill` and any command
  that greps credentials out of the live `.env`, which rules out the usual post-hotfix
  stale-lsnode kill and direct live-DB queries. Two substitutes that worked: (1)
  `tmp/restart.txt` IS the reload mechanism - after it, `logs/auto-refresh.log` showing a
  fresh "scheduler started" line plus a "light OK" pass is the proof the writer came back
  on new code, no process kill needed; (2) for a display change, the public API itself can
  discriminate old vs new code with no DB access: both providers' rows for the SAME
  `api_id` serve one identical `start_time` under the canonical-kickoff read layer, but two
  different bookmaker listing times under the old one - and calibrate the host's datetime
  serialization first against a fixture whose wall-clock kickoff you know from the local
  mirror (live serializes EAT wall-clock to true UTC instants). Note: local and live
  `matches.id` spaces diverge between syncs - never match rows across sides by match id,
